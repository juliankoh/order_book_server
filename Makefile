...
update-peers: ## Fetch latest gossip peer list from API + seed peers
	...
	try: \
	    r = subprocess.run(['curl','-sf','-m','10','-X','POST','--header','Content-Type: application/json','--data','{\"type\":\"gossipRootIps\"}','https://api.hyperliquid.xyz/info'], capture_output=True, text=True, check=True); \
	...
	config = {'root_node_ips': [{'Ip': ip} for ip in all_ips], 'try_new_peers': True, 'chain': '$(CHAIN)'}; \
