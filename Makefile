...
update-peers: ## Fetch latest gossip peer list from API
	@echo "Fetching latest gossip peers..."
	@curl -sf -m 10 -X POST --header "Content-Type: application/json" \
		--data '{ "type": "gossipRootIps" }' https://api.hyperliquid.xyz/info \
		| python3 -c "import json,sys; ips=json.load(sys.stdin); print(json.dumps({'root_node_ips':[{'Ip':ip} for ip in ips],'try_new_peers':True,'chain':'$(CHAIN)'},indent=2))" \
		> $(GOSSIP_CONFIG).tmp \
		&& mv $(GOSSIP_CONFIG).tmp $(GOSSIP_CONFIG) \
		&& echo "Gossip config updated: $(GOSSIP_CONFIG)" \
		|| (rm -f $(GOSSIP_CONFIG).tmp && echo "Failed to fetch gossip config")
